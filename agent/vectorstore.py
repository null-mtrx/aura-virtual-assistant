...
class VectorStoreHandler:
    ...
    def add_to_knowledge_base(self, data: dict, type: Literal["task", "query"]):
        type_data = {"type": type}

        documents = [data[entry]["content"] for entry in data]
        ids = [data[entry]["id"] for entry in data]
        metadata = [metadata|type_data for metadata in list(data.values())]
    ...
    def query(self, query: str, count: int = 5):
        results = self.collection.query(
            query_texts=query, n_results=count
        )
